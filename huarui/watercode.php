...
session_start();
$type = array("login");
if( isset($_GET['t']) && in_array($_GET['t'],$type) ){
	$tcode = $_GET['t'];	
} else {
	die('Error');	
}
...
if($enablegd)
{
	...
	$_SESSION[$tcode]= $string = substr($radomstring,0,4);
...
else
{
	...
	$_SESSION[$tcode] = substr(basename($checkcodefile), 0, 4);
